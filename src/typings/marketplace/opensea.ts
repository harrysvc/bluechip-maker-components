...
import { ECSignature, HowToCall } from 'wyvern-js/lib/types';
...
export interface OpenSeaEventsContact {
  approved_account: unknown | null;
  asset: OpenSeaAsset;
 ...
 */
export interface OpenSeaFungibleToken extends Omit<Token, 'name' | 'symbol' | 'decimals' | 'address'> {
  id?: number;
  symbol?: string;
  address?: string;
  name?: string | null;
...
export interface WyvernOrder {
  ...
  maker: string;
  taker: string;
  maker_relayer_fee: BigNumber;
  taker_relayer_fee: BigNumber;
  maker_protocol_fee: BigNumber;
  taker_protocol_fee: BigNumber;
  fee_recipient: OpenSeaAccount;
  fee_method: number;
  side: number;
  sale_kind: number;
  target: string;
  ...
  base_price: string;
  extra: BigNumber;
  listing_time: BigNumber;
  expiration_time: BigNumber;
  salt: BigNumber;
}
...
export interface Order extends UnsignedOrder, Partial<ECSignature> {
  ...
  maker_account?: OpenSeaAccount;
  taker_account?: OpenSeaAccount;
  payment_token_contract?: OpenSeaFungibleToken;
  payment_token?: OpenSeaFungibleToken;
  fee_recipient_account?: OpenSeaAccount;
