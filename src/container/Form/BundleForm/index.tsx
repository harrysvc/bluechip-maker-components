...
import { StyledFormControl } from './BundleForm.styled';

export type RBundleFormProps = {
  bundleName?: string;
  ...
  expirationTime?: Date;
  listingTime?: Date;
  features?: 'endingPrice' | 'scheduleFutureTime' | 'none';
};

const RBundleForm = ({ paymentTokens, ...props }: FormikConfig<RBundleFormProps> & { paymentTokens: OpenSeaFungibleToken[] }) => {
  const { status } = useSelector((state) => state.nft.sell);

  const validateSchema = Yup.object().shape({
    bundleName: Yup.string().required('Please provide a bundle name'),
    ...
    endAmount: Yup.mixed().when('features', {
      is: 'endingPrice',
      then: Yup.number().lessThan(Yup.ref('startAmount'), 'Ending price must be less than starting price').required(),
    }),
    expirationTime: Yup.mixed().when('features', {
      is: 'endingPrice',
      then: Yup.string().required('Please provide a expiration date'),
    }),
  ...
  const renderSubForm = useCallback(({ values, errors, touched, handleBlur, handleChange }: FormikProps<RBundleFormProps>) => {
  ...
  return (
    <Formik {...props} validationSchema={validateSchema}>
      {(props) => {
        const { values, handleSubmit, errors, touched, setFieldTouched, handleChange, handleBlur, setFieldValue } = props;
        return (
          <>
            <Box mt={5} display="flex" flexDirection="column" gap={2}>
              <RInput
                ...
                onChange={handleChange}
                error={!!errors.bundleName && !!touched.bundleName}
                helperText={errors.bundleName}
                onBlur={handleBlur}
                fullWidth
              />
              ...
              <RInput
                ...
                value={values.bundleDescription}
                onChange={handleChange}
                onBlur={handleBlur}
                fullWidth
              />
              <RInput
                label="Bundle external link"
                name="bundleExternalLink"
                placeholder="Enter bundle external link"
                value={values.bundleExternalLink}
                onChange={handleChange}
                onBlur={handleBlur}
                fullWidth
              />
